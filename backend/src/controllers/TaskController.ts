...
import { formatError, checkUserAccessibility } from '../helpers';
...
class TaskController {
    ...
    static async edit(req: Request, res: Response): Promise<Response> {
        ...
        const missingValues = fields.filter(key => !(key in req.body));
        const isTrue = ['TRUE', 'T', 'Y', 'YES', '1'];
        const isFalse = ['FALSE', 'F', 'N', 'NO', '0'];
        try {
            if (missingValues.length > 0)
                throw formatError(400, `${missingValues.join(', ')} are missing`);
            let task = await prisma.task.findUnique({
            ...
            if (!task) throw formatError(404, 'Task not found');
            if (isTrue.indexOf(isComplete.toUpperCase()) >= 0) isCompleteBool = true;
            else if (isFalse.indexOf(isComplete.toUpperCase()) >= 0) isCompleteBool = false;
